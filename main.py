...
def calculate_value_distribution(parameter_dict_1, parameter_dict_2, parameter_dict_distribution):
    parameter_list =[]
    parameter_list.append(parameter_dict1['latest_revenue'])
...
with st.beta_expander('How to Use'):
    ...
    ticker_input = st.text_input('Please enter your company')
    status_radio = st.radio('Please click "Search".', ('Entry', 'Search'))
...
with st.beta_expander('Monte Carlo Simulation'):
    ...
    with col1:
        ...
        radio_button_revenue_growth = st.radio('Choose Growth Distribution',('Normal','Triangular','Uniform'))

        if radio_button_revenue_growth =='Normal':
            mean_input = st.number_input('Mean Revenue Growth Rate (in%)')
            stddev_input = st.number_input('Revenue Growth Rate STD (in %)')
            parameter_dict_1['revenue_growth'] = mean_input
            ...
            parameter_dict_1['revenue_growth'] = lower_input
            parameter_dict_1['revenue growth'] = upper_input
            prameter_dict_distribution['revenue growth']='uniform'
    ...
    with col2:
        ...
        if radio_button_ebit_margin == 'Normal':
            ...
            stddev_input = st.number_input('EBIT margin stdev.dev. (in%)')
            parameter_dict_1['ebit margin'] =mean_input
            parameter_dict_2['ebit margin'] =stddev_input
            parameter_dict_distribution['ebit margin'] ='normsl'

        elif radio_button_ebit_margin =='Triangular':
            lower_input =st.number_input('Lower and EBIT margin (in%)')
            ...
            parameter_dict_1['ebit_margin'] = lower_input
            parameter_dict_2['ebit_margin'] = upper_input
            parameter_dict_distribution['ebit_margin']='uniform'
    ...
    with col3:
        ...
        if radio_button_tax_rate =='Normal':
            ...
            stddev_input = st.number_input('Tax rate Standard Deviation (in%)')
    ...
    with col21:
        ...
        if radio_button_tax_rate =='Normal':
            ...
            stddev_input = st.number('Capex Ratio Standard Deviation (in%)')
            ...
            parameter_dict_2['capex ratio'] =mean_input
            parameter_dict_distribution['capex ratio'] = 'triangular'

        elif radio_button_tax_rate =='Uniform':
            lower_input =st.number_input('Lower end capex ratio (in%)')
            upper_input = st.number_input('Upper and capex ratio (in%)')
            parameter_dict_1['capex ratio'] = lower_input
            ...
            if inputs_radio == 'Search':
                parameter_dict_1['latest revenue'] = company.income_statement.loc[
                    'totalRevenue', company.income_statement.columns[-1]]
